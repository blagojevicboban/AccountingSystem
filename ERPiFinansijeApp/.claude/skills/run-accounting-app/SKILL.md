...
description: Build, launch, and drive the ERPiFinansijeApp WPF desktop app (screenshot, click, type, log in) via a UI Automation PowerShell driver. Use when asked to run, start, build, test, or screenshot ERPiFinansijeApp / ERPiFinansije, or to verify a WPF/XAML UI change actually works.
---

Paths below are relative to `ERPiFinansije/` (the repo root — it contains
`ERPiFinansije.slnx`). ERPiFinansijeApp is a .NET 8 WPF desktop app (`net8.0-windows`,
...
Unlike some sibling WPF apps in this environment, `ERPiFinansijeApp` has **no
settings.json / user-configurable db path**. `AppConfig.DbPath` (`ERPiFinansijeApp/AppConfig.cs`)
always resolves to `C:\KNJIGE\Radni\KOR01\accounting_kor01.db` if that file exists,
falling back to `%LocalAppData%\ERPiFinansijeApp\Baze\*.db` only if it doesn't. KOR01
...
```powershell
dotnet run --project ERPiFinansijeMigration/ERPiFinansijeMigration.csproj
```

This wipes and re-imports `accounting_kor01.db` from the legacy DBF files (idempotent,
~10s). If `ERPiFinansijeApp` is ever pointed at real customer data at that same fixed
...
```powershell
dotnet build ERPiFinansije.slnx -c Debug
```

Produces `ERPiFinansijeApp\bin\Debug\net8.0-windows\ERPiFinansijeApp.exe`. (Ignore the
...
```
powershell -ExecutionPolicy Bypass -File "C:\ERPi\ERPiFinansije\ERPiFinansijeApp\.claude\skills\run-accounting-app\driver.ps1"
```
...
powershell -ExecutionPolicy Bypass -File "C:\ERPi\ERPiFinansije\ERPiFinansijeApp\.claude\skills\run-accounting-app\driver.ps1" close
```
...
Visual Studio 2022+ / Rider: open `ERPiFinansije.slnx`, set `ERPiFinansijeApp` as
startup project, F5. Or `dotnet run --project ERPiFinansijeApp\ERPiFinansijeApp.csproj`
...
- **`BtnUvozDOS` ("DOS Legacy Import") does not actually run a migration.** It's
  ...
  the separate `ERPiFinansijeMigration` console project (`dotnet run --project
  ERPiFinansijeMigration/ERPiFinansijeMigration.csproj`). Don't mistake clicking it
...
Fix (`ERPiFinansijeApp/Views/Nalozi/NaloziView.xaml` + `.xaml.cs`): removed the
