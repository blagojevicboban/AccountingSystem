...
public partial class TrgovinaView : UserControl
{
    ...
    private async void LoadMagacineIRobneKartice()
    {
        try
        {
            ...
            var stavkeZaCombo = new List<ERPiFinansijeData.Models.Magacin> { SviMagaciniOpcija };
    ...
    {
        if (CmbMagacinRobno.SelectedItem is not ERPiFinansijeData.Models.Magacin magacin)
        {
    ...
    private static readonly ERPiFinansijeData.Models.Magacin SviMagaciniOpcija = new()
    {
        MagacinId = -1,
        SifraMagacina = "*",
    ...
    private static bool JeSviMagacini(ERPiFinansijeData.Models.Magacin? m) => m == null || m.MagacinId == -1;

    private static string SifraZaFajl(ERPiFinansijeData.Models.Magacin m) => JeSviMagacini(m) ? "SVI" : m.SifraMagacina;
    ...
    private static async Task<List<(ERPiFinansijeData.Models.Magacin Magacin, Artikal Artikal, List<MaterijalnaKartica> Kartice)>> PrikupiRobneKarticeAsync(
        AccountingDbContext db, ERPiFinansijeData.Models.Magacin? magacinFilter, IReadOnlyCollection<Artikal>? artikliFilter)
    {
        var magaciniZaObradu = magacinFilter == null
            ? await db.Magacini.OrderBy(m => m.SifraMagacina).ToListAsync()
            : new List<ERPiFinansijeData.Models.Magacin> { magacinFilter };

        var sifreFiltera = artikliFilter?.Select(a => a.SifraArtikla).ToHashSet(StringComparer.OrdinalIgnoreCase);
        var artikliDict = await db.Artikli.ToDictionaryAsync(a => a.SifraArtikla, a => a);
        var rezultat = new List<(ERPiFinansijeData.Models.Magacin, Artikal, List<MaterijalnaKartica>)>();
    ...
    private List<ERPiFinansijeData.Models.Magacin> _sviRacunopolagaci = new();
    ...
    {
        if (DgRacunopolagaci.SelectedItem is not ERPiFinansijeData.Models.Magacin selektovan)
        {
    ...
    private async void StampajPrimopredaju(DataGrid dgNalozi)
    {
        ...
        try
        {
            ...
            var firma = await db.Firme.FirstOrDefaultAsync() ?? new Firma { Naziv = "Preduzeće" };
            var magDaje = await db.Magacini.FirstOrDefaultAsync(m => m.SifraMagacina == fullNalog.SifraMagacinaDaje) ?? new ERPiFinansijeData.Models.Magacin { SifraMagacina = fullNalog.SifraMagacinaDaje, NazivMagacina = fullNalog.SifraMagacinaDaje };
            var magPrima = await db.Magacini.FirstOrDefaultAsync(m => m.SifraMagacina == fullNalog.SifraMagacinaPrima) ?? new ERPiFinansijeData.Models.Magacin { SifraMagacina = fullNalog.SifraMagacinaPrima, NazivMagacina = fullNalog.SifraMagacinaPrima };
    ...
    private async void LoadRobniBrutoBilans()
    {
        try
        {
            ...
            {
                var magacini = await db.Magacini.ToListAsync();
                magacini.Insert(0, new ERPiFinansijeData.Models.Magacin { MagacinId = 0, SifraMagacina = "SVI", NazivMagacina = "--- Svi magacini ---" });
