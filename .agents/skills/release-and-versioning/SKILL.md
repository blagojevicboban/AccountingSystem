...
3. Check for EF Core schema migrations in `AccountingData/Migrations/` added since the previous version tag (`git diff --name-only <last_tag> HEAD AccountingData/Migrations/`). If any exist, flag a **`⚠️ OBAVEŠTENJE O MIGRACIJI BAZE`** section in the release notes.
...
1. Stage modified files: `git add version.txt CHANGELOG.md README.md AccountingApp/ AccountingData/ .agents/`.
